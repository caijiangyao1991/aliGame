...
df_test = pd.read_csv('./rawData/tmp_train_new_t.csv',dtype={'pid':np.str})

days = df_test[['pid','jtime','SUM(df_train.本次审批金额)','SUM(df_train.药品费发生金额)','SUM(df_train.检查费发生金额)','SUM(df_train.非账户支付金额)']]
...
# days_final.to_csv('./data/person_oneMonthSumCost_t.csv',index=False)
...
# print(days_final.head())
# days_final.to_csv('./data/person_oneMontDrugCost_t.csv',index=False)
#项目和非账户支付金额
programCost = []
selfpyCost = []
for id in pid:
...
days_program.to_csv('./data/person_oneMonthProgramCost_t.csv',index=False)
...
days_selfpay.to_csv('./data/person_oneMonthSelfpayCost_t.csv',index=False)